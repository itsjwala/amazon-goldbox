...
const DEFAULT_IMAGE = process.env.default_image_link 
...
class Product {
  ...
  setupItemFromXml(data) {
    ...
    // Upgrade the resolution of Amazon product images

    if ( typeof this.image === 'string' ) {
      this.image = this.image.replace('._SL160_.jpg', '.jpg');
    }

    if ( !this.image || this.image.includes('no-img') ) {
...
function productDescription(description) {
  ...
  desc = keywordify(desc, accounts, '@');
  desc = keywordify(desc, hashtags, '#');
