...
function getDealsFeedFromScripts(scripts) {
  ...
  scripts.forEach(function(widget) {
    ...
    const { prefetchedData } = deals;

    if ( !prefetchedData ) return;

    const dealDetails = prefetchedData.dcsGetDealsList[0].dealDetails;

    let dealList = Object.keys(dealDetails).map(key => dealDetails[key]);

    dealList = dealList.filter(({ title }) => title);

    for ( let key in dealList ) {

      const item = setupProductFromJson(dealList[key]);
...
function parseScriptStringToJson(data) {
  const regex = new RegExp(/\s+assets\.mountWidget\('slot-6',\s*(\{[\S\s]*\})\)\s+/);

  const matches = data.match(regex);
  let deals;

  try {
    deals = matches && JSON.parse(matches[1]);
  } catch(e) {
...
function filterWidgetsByData(item) {
  return item.data.includes('widgetToRegister') || item.data.includes('dealDetails') || item.data.includes('widgetId');
}
